...
export class Guard {
  ...
  static ensureIsDate(value: unknown, propertyName: string) {
    if (!(value instanceof Date) || value.toString() === 'Invalid Date') {
      throw new InvalidValueException(`${propertyName} must be a date`);
    }
  }

  static ensureDateIsNotInFuture(value: Date, propertyName: string) {
    if (value > new Date()) {
      throw new InvalidValueException(`${propertyName} cannot be in the future`);
    }
  }
}
